...
from interfaces import  MatchingType
from Map import *
...
#Be Cautious!!
#Psycopg2 doesn't support Jython, so you have to find another way in order to save and obtain shortest path information without psycopg2!
...
class Matching(MatchingType):
	...
	def point_matching(self, devicesn, timestamp, lon, lat, spd):
		traj_point = TrajPoint(devicesn, timestamp, lon, lat, spd)
		...
		if (mod_road_id, mod_seg_id) == (-1, -1) or (mod_road_id, mod_seg_id) == self.supp[dsn]["prev_seg"]:
			...
			return str(road_id) + '_' + str(seg_id) + '_' + '-1' + '_' + '-1'
		else:	
			self.supp[dsn]["prev_prev_seg"] = (mod_road_id, mod_seg_id)
			self.supp[dsn]["prev_seg"] = (road_id, seg_id)
			return str(road_id) + '_' + str(seg_id) + '_' + str(mod_road_id) + '_' + str(mod_seg_id)
